%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function:
%        批量处理音频数据，提取小波能量特征并将数据存储在excel表格中
%        
% Author:
%        LiXinRui
% Version:
...
%输入数据
path="F:\document\海天探索者\水下信标\声音信号\鲸鱼叫声";
Files = dir(fullfile(path,'*.mp3'));
n=length(Files);
data=cell(n,9);
for index=1:n
    close all;
    ...
    [x,fs]=audioread(path+'\'+signal_name);
    plot(x);
